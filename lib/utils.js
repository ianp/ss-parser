const debug = require('debug-levels')('ss-parser:utils')
const debugMT = require('debug-levels')('ss-parser:utils:mini-topic')
var fs = require("fs");
...
var trim = exports.trim = function (text) {
  ...
  if (before !== text) {
    debug.verbose("trimmed", {before, after: text})
  }
  return text;
};
...
exports._searchMiniTopic = function (cursor, cmd, instructions) {
  ...
  for (var i = cursor; i < instructions.length; i++) {
    ...
    debugMT.verbose('process line = %s, look cmd = %s (%s), lookahead = %d', instructions[i], lookCmd, cmd, lookahead)
    ...
    if (lookahead.length !== 0) {
      ...
      if (cmd === "+" || cmd === "?") {
        if (lookCmd === "%") {
          debugMT.verbose('setting isPrevious = %s', lookahead)
...
var walk = function (dir, done) {
  ...
  if (fs.statSync(dir).isFile()) {
    debug.info('expected directory but found file, simulating directory: %s', dir);

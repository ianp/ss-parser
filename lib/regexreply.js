...
var debug = require('debug-levels')('ss-parser:regex-reply')
...
exports.parse = function (regexp, facts, callback) {
  ...
  var mmWidthStarReplace = function (match, p1) {
    ...
    if (parts.length === 2) {
      ...
      if (v1 === v2) {
        debug.warn('MM Values are the same, dropping back to Variable Match')
    ...
    facts.conceptToList(word.toLowerCase(), function (err, words) {
      ...
      if (!_.isEmpty(words)) {
        ...
        wordnet.lookup(word, sym, function (err2, words) {
          ...
          if (_.isEmpty(words)) {
            debug.warn('creating a trigger with a word NOT EXPANDED', match)
            done(null, match);
          } else {

            words = "(\\b" + words.join("\\b|\\b") + "\\b)";
            debug.info('wordnet replies', words)
            done(null, words);
          }
        });
      }
  ...
  replace(regexp, /(~)(\w[\w]+)/g, wordnetReplace, function (err, result) {
    ...
    while (match) {
      giveup++;
      if (giveup >= 50) {
        debug.warn('Infinite loop when trying to process optionals in trigger!')
        return "";
      }
